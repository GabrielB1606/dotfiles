return {
  ...
  keys = {
    ...
    { "<C-p>", "<cmd>Telescope find_files<cr>", desc = "Find file Telescope (root dir)" },
    { "<C-m>", "<cmd>Telescope marks<cr>", desc = "Find marks Telescope" },
    {
      "<leader>h",
      function()
    ...
    { "<C-F>", "<cmd>Telescope live_grep<cr>", desc = "Telescope live grep" },
    {
      ...
      function()
        ...
        require("telescope.builtin").live_grep({ default_text = get_visual() })
      end,
      mode = "x",
      desc = "Live Grep selection",
    },
    { "<leader>p", "<cmd>Telescope oldfiles<cr>", desc = "Telescope live grep" },
    {
      "<leader>,",
    ...
    -- Replace all occurrences of the visual selection in the current file
    {
      "<C-r>",
      function()
        local save_reg = vim.fn.getreg('"')
        ...
        local prompt = "Replace all occurrences of '" .. text .. "' with: "
        local replacement = vim.fn.input(prompt)
        if replacement == nil then
          return
        end

        local esc_text = vim.fn.escape(text, "\\/")
        local esc_repl = vim.fn.escape(replacement, "\\/&")

        vim.cmd("%s/\\V" .. esc_text .. "/" .. esc_repl .. "/g")
      end,
      mode = "x",
      desc = "Replace all occurrences of selection in current file",
    },
    -- find
    {
      "<leader>fb",
